...
void fb_flip_horizontal(FrameBuffer* fb)
{
    if (!fb || fb->width <= 1) {
        return;
    }
    ...
    for (uint16_t y = 0; y < fb->height; y++) {
        for (uint16_t x = 0; x < fb->width / 2; x++) {
            uint8_t* left = &fb->pixels[y * fb->width + x];
            uint8_t* right = &fb->pixels[y * fb->width + (fb->width - 1 - x)];
...
void fb_flip_vertical(FrameBuffer* fb)
{
    uint8_t temp_row[fb->width];
    ...
    for (uint16_t y = 0; y < fb->height / 2; y++) {
        uint8_t* top = &fb->pixels[y * fb->width];
        uint8_t* bottom = &fb->pixels[(fb->height - 1 - y) * fb->width];

        // Swap the pixels
        memcpy(temp_row, top, fb->width);
        memcpy(top, bottom, fb->width);
        memcpy(bottom, temp_row, fb->width);
    }
}
